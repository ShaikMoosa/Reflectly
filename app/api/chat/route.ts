...
// Use a dummy key during build time if none is provided
// Note: This client will be non-functional, but at least it won't crash during build
const dummyApiKey = process.env.NODE_ENV === 'production' ? 'dummy-key-for-build-only' : 'dummy-key-for-local-dev';
const apiKey = process.env.OPENAI_API_KEY || dummyApiKey;

// Initialize the OpenAI client with a key, even if it's just a dummy one for build
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      console.error('OpenAI API key is missing. Using the real API requires a valid key.');
      return NextResponse.json(
        { error: 'API configuration error - OpenAI API key is not configured.' },
        { status: 500 }
      );
    }

    // Get user ID from authorization header to track usage
    let userId = null;
    ...
    // Check if the user is authenticated and has available usage quota
    if (userId) {
      const canUseAiChat = await incrementAiChatCount(userId);
      if (!canUseAiChat) {
        return NextResponse.json({
          error: 'You have reached your AI chat usage limit. Please upgrade your plan for unlimited access.',
          limitReached: true
        }, { status: 403 });
      }
    }

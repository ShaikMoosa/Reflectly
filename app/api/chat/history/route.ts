...
export async function POST(request: NextRequest) {
  try {
    ...
    const chatHistoryData: CreateChatHistoryParams = await request.json();
    
    // Create the database object with proper field names
    const dbChatHistoryData = {
      user_id: user.id,
      project_id: chatHistoryData.projectId,
      title: chatHistoryData.title,
      messages: chatHistoryData.messages
    };
    
    // Verify the project exists and is owned by the user
    if (chatHistoryData.projectId) {
      const { data: projectData, error: projectError } = await supabase
        .from('projects')
        .select('id')
        .eq('id', chatHistoryData.projectId)
        .eq('user_id', user.id)
        .single();
